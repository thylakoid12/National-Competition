...
from .storage import read_arrays, write_json, file_hash
...
def make_payload(source, output):
    source, output = Path(source).resolve(), Path(output).resolve()
    variant, directory, daily = daily_results(source)
    ...
    summary_rows = [
        ...
        [
            "模型定义",
            "固定α=1，优化购电计划" if model == "A" else "联合优化购电计划与α",
        ],
        ["比较口径", "A为指定对照；一月选型胜出方案仍为B"],
        ["指标", "数值"],
    ]
    ...
    payload = dict(
        ...
        source=str(source),
        output=str(output),
        template=str(ATTACHMENTS / "附件5/result2.xlsx"),
        summary=summary,
        purchase=purchase,
        storage=storage,
        emergency=emergency,
    ...
    (output / "使用说明.md").write_text(
        ...
        "模型A是用户指定的固定方案对照，不将其写成一月选型胜出者；模型B保留原一月选型结论。\n",
        encoding="utf-8",
    )
    return payload
...
def export_excel(source, output, node=None):
    payload = make_payload(source, output)
