...
from q2_controller import reference_structure

class FastReference:
    def __init__(self,forecast,E0,price,nu,cfg):
        import highspy
        self.highspy=highspy
        self.forecast,self.E0,self.price,self.nu,self.cfg=forecast,E0,price,nu,cfg
        n=len(price)
        self.n=n
        ...
        for k,v in [('output_flag',False),('threads',1),('solver','simplex'),
                    ('random_seed',cfg.seed),('dual_feasibility_tolerance',1e-8),
    ...
    def solve(self,G):
        ...
        upper=np.r_[np.minimum(np.maximum(a,0),cfg.B),np.minimum(np.maximum(-a,0),cfg.B)]
        ...
        residual=max(float(np.max(np.abs(self.A@x))),
                     max(0.,float(self.c@x-first-cfg.objective_tolerance)),
                     max(0.,float(cfg.e_min-x[2*n+1:].min()),float(x[2*n+1:].max()-cfg.e_max)),
                     max(0.,float((x[:2*n]-upper).max()),float(-x[:2*n].min())))
        if residual>cfg.physical_tolerance:
            raise RuntimeError('Native reference residual: '+str(residual))
        return x[2*n:],dict(primary_status=0,secondary_status=0,residual=residual,
                  primary_objective=float(first+cfg.emergency_multiplier*self.price@np.maximum(-a,0)+self.nu*cfg.e_min),
                  backend='highspy-'+self.h.version(),deterministic_basis_reset=True)

def _batch(G,reference,E0,paths,price,nu,e_min,e_max,B,eta_c,eta_d,multiplier):
    ...
    for s in range(len(paths)):
        ...
        for t in range(len(G)):
            ...
            C=min(max(a,0.),B,max((e_max-E)/eta_c,0.))
            D=min(max(-a,0.),B,eta_d*max(E-reference[t+1],0.))
            cost+=multiplier*price[t]*(max(-a,0.)-D)
            E+=eta_c*C-D/eta_d
...
def fast_batch(G,reference,E0,paths,price,nu,cfg):
    ...
    return _compiled(G,reference,E0,paths,price,nu,cfg.e_min,cfg.e_max,cfg.B,
